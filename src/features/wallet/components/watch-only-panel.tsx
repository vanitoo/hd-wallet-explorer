...
import { DISCOVERY_STORAGE_KEY,PROFILE_STORAGE_KEY,isDiscoverySnapshot,parseStoredArray,type DiscoverySnapshot } from "../domain/workspace";

function readProfiles(){return typeof window==="undefined"?[]:parseWatchProfiles(localStorage.getItem(PROFILE_STORAGE_KEY))}
function endpointForKey(value:string){try{return inspectWatchKey(value).network==="mainnet"?"https://mempool.space/api":"https://mempool.space/testnet/api"}catch{return "https://mempool.space/api"}}
...
export function WatchOnlyPanel(){
  ...
  const[profiles,setProfiles]=useState<WatchProfile[]>(readProfiles); const[error,setError]=useState<string|null>(null);
  ...
  function changeKey(value:string){setKey(value);setEndpoint(endpointForKey(value));resetResults()}
  ...
  function save(){try{const profile=createWatchProfile(name,key);const next=[profile,...profiles.filter(item=>item.extendedPublicKey!==profile.extendedPublicKey)];setProfiles(next);localStorage.setItem(PROFILE_STORAGE_KEY,JSON.stringify(next));setName("");setError(null)}catch(e){setError(e instanceof Error?e.message:"Не удалось сохранить профиль")}}
  function load(profile:WatchProfile){setKey(profile.extendedPublicKey);setEndpoint(endpointForKey(profile.extendedPublicKey));setName(profile.name);resetResults()}
  function remove(id:string){const next=profiles.filter(item=>item.id!==id);setProfiles(next);localStorage.setItem(PROFILE_STORAGE_KEY,JSON.stringify(next))}
  function saveDiscovery(result:DiscoveryResult){
    const snapshots=parseStoredArray(localStorage.getItem(DISCOVERY_STORAGE_KEY),isDiscoverySnapshot);
    const snapshot:DiscoverySnapshot={
      id:globalThis.crypto?.randomUUID?.()??`${Date.now()}-${Math.random().toString(16).slice(2)}`,
      profileName:name.trim()||profiles.find(item=>item.extendedPublicKey===key.trim())?.name||`${info?.prefix.toUpperCase()??"Bitcoin"} wallet`,
      extendedPublicKey:key.trim(),network:result.network,checked:result.checked,used:result.used,errors:result.errors,
      balanceSats:Number(result.balanceSats),createdAt:new Date().toISOString(),
      addresses:result.branches.flatMap(branch=>branch.rows.map(row=>({address:row.address,branch:branch.label,index:row.index,balance:row.balance,transactionCount:row.transactionCount,status:row.status}))),
    };
    localStorage.setItem(DISCOVERY_STORAGE_KEY,JSON.stringify([snapshot,...snapshots].slice(0,50)));
  }
  async function runDiscovery(){
    ...
    try{
      ...
      setDiscovery(result);saveDiscovery(result);setMessage(`Готово. Проверено ${result.checked}, использовано ${result.used}. Результат сохранён в Workspace.`);setProgress(100);
  ...
  return <div className="watch-only">
    ...
    <label>Расширенный публичный ключ<textarea value={key} onChange={e=>changeKey(e.target.value)} placeholder="xpub… / ypub… / zpub… / tpub… / upub… / vpub…"/></label>
