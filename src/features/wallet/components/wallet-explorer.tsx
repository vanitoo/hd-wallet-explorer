...
export function WalletExplorer() {
  ...
  function resetOutput() {
    ...
    setResult(null); setRows([]); setScannedRows([]); setScanProgress(0); setScanning(false); setScanMessage(""); setError(null); setShow(false);
  }

  function switchChain(next: Chain) {
    setChain(next); resetOutput(); setNetworkEnabled(false);
  ...
  function derive() {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Ошибка деривации"); }
  }
  ...
  function deriveRange() {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Ошибка генерации диапазона"); }
  }

  async function scan() {
    if (!networkEnabled) { setError("Подтвердите отправку публичных адресов в выбранный API."); return; }
    const controller = new AbortController();
    abortRef.current = controller;
    setScanning(true); setScannedRows([]); setScanProgress(0); setError(null); setScanMessage("Подключение к API…");
    try {
      const values = await scanAddressRows({
        chain, endpoint, rows, gapLimit, retries, timeoutMs: timeoutSeconds * 1000, requestDelayMs: requestDelay, signal: controller.signal,
  ...
  return <>
    <section className="hero"><h1>HD Wallet Explorer</h1><p>Создавайте адреса локально и проверяйте балансы через выбранный публичный или собственный RPC. Секретная фраза и приватные ключи не покидают браузер.</p></section>
    <section className="panel">
      <div className="tabs"><button className={chain === "ethereum" ? "active" : ""} onClick={() => switchChain("ethereum")}>Ethereum</button><button className={chain === "bitcoin" ? "active" : ""} onClick={() => switchChain("bitcoin")}>Bitcoin</button></div>
      ...
      <div className="warning">Используйте демо-фразу для проверки. Реальную seed-фразу вводите только на доверенном устройстве. Сканер отправляет в API исключительно публичные адреса.</div>
...
function NumberField({ label, value, min, max, onChange }: { label: string; value: number; min: number; max?: number; onChange: (value: number) => void }) { return <label>{label}<input type="number" value={Number.isFinite(value) ? value : ""} min={min} max={max} onChange={(event) => onChange(event.target.valueAsNumber)} /></label>; }
